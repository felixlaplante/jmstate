...
class MetropolisHastingsSampler:
    ...
    pdf_fn: Callable[[Tensor2D], tuple[Tensor1D | Tensor2D, Any]]
    init_state: Tensor2D
    n_chains: int
    ...
    def __init__(
        self,
        pdf_fn: Callable[[Tensor2D], tuple[Tensor2D, Any]],
        init_state: Tensor2D,
        n_chains: int,
        init_step_size: float,
        adapt_rate: float,
        ...
        Args:
            pdf_fn (Callable[[Tensor2D], tuple[Tensor1D | Tensor2D, Any]]): pdf with aux.
            init_state (Tensor2D): Starting state for the chain.
        ...
        """
        self.pdf_fn = torch.vmap(pdf_fn)  # type: ignore
        self.n_chains = n_chains
        self.adapt_rate = adapt_rate
        self.target_accept_rate = target_accept_rate

        # Initialize state
        self.current_state = init_state.unsqueeze(0).repeat_interleave(n_chains, dim=0)
        ...
        self.step_sizes = torch.full(
            (init_state.shape[0],), init_step_size, dtype=torch.float32
        )

        # Statistics tracking
        self.n_samples = torch.tensor(0, dtype=torch.int64)
        self.n_accepted = torch.zeros((init_state.shape[0],), dtype=torch.float32)
    ...
    def step(self) -> tuple[tuple[Tensor2D, Tensor2D], Any]:
        """Performs a single kernel step.

        Returns:
            tuple[Tensor2D, Tensor2D]: A tuple containing state and pdf.
        """
        ...
        # Get the proposal
        proposed_state = self.current_state + noise * self.step_sizes.unsqueeze(1)
