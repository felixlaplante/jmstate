...
from ..utils._defs import TWO, BaseHazardFn, LinkFn, Trajectory
...
class HazardMixin:
    ...
    def _cum_hazard(
        ...
        def _quad_create():
            return torch.addmm(
                0.5 * (c + t1), half.view(-1, 1), self._std_nodes.view(1, -1)
            )

        quad = (
            self._get_cache("quad", key, _quad_create)
            if enable_cache
            else _quad_create()
        )
    ...
    def _log_and_cum_hazard(
        ...
        def _quad_create():
            return torch.addmm(
                0.5 * (t0 + t1), half.view(-1, 1), self._std_nodes.view(1, -1)
            )
    ...
    def compute_surv_log_probs(
        ...
        # Check dims
        if u.ndim != TWO or u.shape[0] != sample_data.size:
            raise ValueError(
