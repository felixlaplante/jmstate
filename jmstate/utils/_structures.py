...
from ._utils import build_vec_rep, log_cholesky_from_flat
...
class ModelData:
    ...
    def _check(self):
        ...
        if self.x is not None and self.x.ndim != TWO:
            raise ValueError(f"x must be None or 2D, got {self.x.ndim}D")
        if self.y.ndim != THREE:
...
class SampleData:
    ...
    def _check(self):
        ...
        # Check dimensions
        if self.c is not None and self.c.ndim != ONE:
            raise ValueError(f"c must be 1D, got {self.c.ndim}D")
        if self.x is not None and self.x.ndim != TWO:
            raise ValueError(f"x must be None or 2D, got {self.x.ndim}D")
        if self.psi.ndim != TWO:
