...
from ..typedefs._structures import AllInfo, BaseInfo, Job, ModelParams, SampleData
...
class PredictY(Job):
    ...
    def run(self, info: AllInfo, metrics: dict[str, Any]) -> None:
        psi = info.model.model_design.individual_effects_fn(
            gamma=info.model.params_.gamma, x=info.data.x, b=info.b
        )
...
class PredictSurvLogps(Job):
    ...
    def run(self, info: AllInfo, metrics: dict[str, Any]) -> None:
        ...
        sample_data = SampleData(info.data.x, info.data.trajectories, psi, info.data.c)
...
class PredictTrajectories(Job):
    ...
    def run(self, info: AllInfo, metrics: dict[str, Any]) -> None:
        ...
        sample_data = SampleData(self.x_rep, self.trajectories_rep, psi_rep, self.c_rep)
        ...
        metrics["pred_trajectories"].append(trajectory_chunks)
